...
\usetikzlibrary{shadows, arrows.meta,decorations.markings}
\tikzset{
  tokenstyle/.style={
    rectangle,
    rounded corners,
...
We select a starting node from which the search begins, typically the WETH token.
...
Next, we remove this node from the graph and compute the connected components. After adding the starting node back to the graph, we yield subgraphs that are disjoint except for the starting node. It is easy to see that cycles can be searched independently in these subgraphs, because any cycle that would traverse multiple components could only cross via the starting node. In such a case, however, the cycle can be split into smaller cycles that each remain within a single component.
...
See also Fig. \ref{fig:token_graph} showing the graph skeleton of the largest component, which is a simplified representation produced by first removing self-loops and iteratively pruning leaves (degree‑1 nodes), then collapsing degree‑2 chains into single edges marked as skeleton (and mark these edges as dashed lines). This preserves the graph for cycle search while suppressing peripheral noise and intermediate chain nodes for clearer visualization and analysis.
...
In this subrutine we assume $\xin$ is given, and a subgraph $G=(V,E)$. The task is to find cycles such that are profitable without considering gas, i.e.  $w_3(w_2(w_1(\xin))) > \xin$ on Fig. \ref{fig:arbcycle}.  
...
\begin{enumerate}[leftmargin=*, label=\arabic*.]
    ...
    If this results in more tokens than currently recorded at $d_v$, then we need to update our data structures as follows: If the path $P_u$ dos not traverse token $v$, we update $d_v$ and $P_v$, otherwise we update  $d^1_v$ and $P^1_v$. In the later case we have found a path from $s$ and with a loop through $v$, which we save as a candidate. Finally, if node $v=s$, we have found a loop.
...
The function \texttt{save\_cycle}$(\xin, P)$ saves a profitable arbitrage cycle with a given amount $\xin$. While the function 
